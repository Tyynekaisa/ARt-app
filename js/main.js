...
async function startAR() {
    ...
    const mindARThreeJs = new window.MINDAR.IMAGE.MindARThree({
        ...
        imageTargetSrc: "ARt-app/assets/targets/gemstone_targets.mind",
        maxTrack: 3
    })
    ...
    const envMap = textureLoader.load('ARt-app/assets/textures/photostudio.jpg')
    ...
    for (const key in gemstones) {
        ...
        // 3D-mallit
        const model = await loadGLTF(`ARt-app/assets/models/gemstones/${item.model}`)
